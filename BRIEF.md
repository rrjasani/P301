...
A page-level filter row (form, tradition, disposition,
with a reset and a live "N of M SKUs" note) sits above the header KPIs and
scopes everything beneath it. Header KPIs sit below that and stay visible;
sections 2–6 are organized as **tabs** below the KPIs (not a single scrolling
page) so Maya can jump straight to what she cares about. The SKU drawer is a
global overlay reachable from any tab.
...
1. **Header** — assortment health at a glance: total SKUs, total capital tied up,
   ...
   All five tiles recompute live against whatever the filter row above has
   narrowed the assortment to.
